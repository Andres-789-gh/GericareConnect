...
// Incluimos las clases que necesitamos para trabajar con usuarios y pacientes.
require_once __DIR__ . '/../../../models/clases/usuario.php';
require_once __DIR__ . '/../../../models/clases/pacientes.php';

// Creamos un objeto 'usuario' para usar su función y traer a los familiares.
...
// Si la URL tiene un ID, activamos el "modo edición".
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $modo_edicion = true;
    $pacienteModel = new Paciente();
    // Obtenemos los datos del paciente para rellenar el formulario.
...
<head>
    ...
    <style>
        :root {
            --primary-color: #007bff; --light-gray: #f8f9fa; --medium-gray: #dee2e6;
            --dark-gray: #6c757d; --text-color: #212529; --white: #ffffff;
            --shadow: 0 4px 15px rgba(0, 0, 0, 0.08); --success-color: #28a745;
        }
        body {
    ...
    <div class="container">
        ...
        <form action="../../../controllers/admin/paciente_controller.php" method="POST">
            ...
            <div class="form-grid">
                ...
                <div class="form-group full-width"><label>Seguro Médico (EPS)</label><input type="text" name="seguro_medico" value="<?= htmlspecialchars($datos_paciente['seguro_medico'] ?? '') ?>"></div>
                <div class="form-group full-width"><label>Número de Afiliación</label><input type="text" name="numero_seguro" value="<?= htmlspecialchars($datos_paciente['numero_seguro'] ?? '') ?>"></div>
                
                <div class="form-group full-width">
                    <label><i class="fas fa-link"></i> Enlazar Familiar (Opcional)</label>
                    <select name="id_usuario_familiar">
