...
-- Crea el nuevo procedimiento que ahora acepta un parámetro de búsqueda
...
BEGIN
    SELECT
        ...
        hc.estado_salud,
        p.documento_identificacion -- Se añade para la búsqueda
    FROM
    ...
    WHERE
        hc.estado = 'Activo'
        -- Si se busca por ID de historia, se ignora el texto de búsqueda
        AND (p_id_historia_clinica IS NULL OR hc.id_historia_clinica = p_id_historia_clinica)
        -- Si no se busca por ID, se usa el texto de búsqueda
        AND (p_id_historia_clinica IS NOT NULL OR p_busqueda IS NULL OR p_busqueda = '' OR
